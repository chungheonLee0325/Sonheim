...
*   **정교한 조준 및 전투 연계**
    ...
    *   **공격 및 회피:** 마우스 클릭과 특정 키를 통해, [[3.3 스킬 시스템|스킬 시스템]]과 연동된 공격 및 회피 스킬을 사용합니다.

*   **특수 이동 및 상태**
    *   **스프린트 (Sprint):** Shift 키를 누르고 있는 동안 [[3.2 어트리뷰트 시스템|스태미나]]를 소모하여 빠르게 달립니다. 클라이언트 예측 기법이 적용되어 키를 누르는 즉시 반응합니다.
...
### 1. 부드러운 캐릭터 회전: `RInterpTo`
조준 모드에서 캐릭터는 항상 카메라가 바라보는 방향을 향해야 합니다. 하지만 `Controller`의 회전값을 `Pawn`에 즉시 적용하면, 마우스를 빠르게 움직일 때 캐릭터가 부자연스럽게 '휙'하고 돌아버립니다. 이 문제를 해결하기 위해 `ASonheimPlayerController`의 `Tick` 함수에서 `FMath::RInterpTo`를 사용하여, 현재 캐릭터의 회전값이 목표 회전값(컨트롤러의 회전값)을 부드럽게 따라가도록 구현했습니다.

> [View on GitHub](https://github.com/chungheonLee0325/Sonheim/blob/main/Sonheim/Source/Sonheim/AreaObject/Player/SonheimPlayerController.cpp#L188)
```cpp
// Source/Sonheim/AreaObject/Player/SonheimPlayerController.cpp
void ASonheimPlayerController::UpdateRotation(float DeltaTime)
{
    // ...
    if (bIsAiming)
    {
        FRotator TargetRotation = GetControlRotation();
        // 현재 Pawn의 회전값을 목표 회전값까지 부드럽게 보간
        TargetRotation = FMath::RInterpTo(Pawn->GetActorRotation(), TargetRotation, DeltaTime, 15.0f);
...
조준 시 카메라가 갑자기 줌인되면 플레이어는 어색함을 느낍니다. `ASonheimPlayer`의 `Tick` 함수에서는 `FMath::FInterpTo`를 사용하여, 카메라를 담고 있는 `SpringArmComponent`의 `TargetArmLength` 값을 목표 값까지 매끄럽게 보간합니다. 이를 통해 조준 시에는 부드럽게 줌인되고, 조준을 풀면 다시 부드럽게 줌아웃되는 카메라 워크를 구현했습니다.

> [View on GitHub](https://github.com/chungheonLee0325/Sonheim/blob/main/Sonheim/Source/Sonheim/AreaObject/Player/SonheimPlayer.cpp#L345)
```cpp
// Source/Sonheim/AreaObject/Player/SonheimPlayer.cpp
void ASonheimPlayer::Tick(float DeltaTime)
{
    // ...
    // 목표 Arm Length (조준 시 150, 평소 400)를 향해 부드럽게 보간
    float TargetArmLength = bIsAiming ? 150.0f : 400.0f;
    CameraBoom->TargetArmLength = FMath::FInterpTo(
        CameraBoom->TargetArmLength, TargetArmLength, DeltaTime, 10.0f);
}
```
