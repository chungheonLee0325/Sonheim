...
> [View on GitHub](https://github.com/chungheonLee0325/Sonheim/blob/main/Sonheim/Source/Sonheim/AreaObject/Player/Utility/InventoryComponent.cpp#L100)
```cpp
...
void UInventoryComponent::GetLifetimeReplicatedProps(...) const
{
    // ...
    // 1. 소유자에게만 복제
    DOREPLIFETIME_CONDITION(UInventoryComponent, RepInventoryItems, COND_OwnerOnly);
}
...
서버 권위 아키텍처는 안정성을 보장하지만, 모든 행동이 서버의 응답을 기다려야 한다면 네트워크 지연(Lag)으로 인해 사용자 경험이 저하될 수 있습니다. Sonheim은 이를 보완하기 위해, UI와 같이 반응성이 중요한 일부 시스템에 **클라이언트 예측** 기술을 제한적으로 적용합니다. (상세: [[8.1 반응형 인벤토리 UX|8.1_Case_Study_Inventory_Interaction]])
