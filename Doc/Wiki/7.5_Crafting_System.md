...
## 7.5.2. 아키텍처: 서버의 상태 라우터와 비동기 작업 루프
...
#### 2. 컨텍스트 기반 상호작용 라우터 (`Interact_Implementation`)
...
```
이 라우팅 로직 덕분에, 플레이어는 제작대의 상태를 인지하지 않고도 `F`키 하나만으로 상황에 맞는 올바른 상호작용을 할 수 있습니다.
...
## 7.5.4. 구현 경험: 동시성 문제 해결 (Mutex-like State Locking)
...
#### 해결 방안: `UIOwner`를 이용한 상태 잠금 (State Locking)
...
*   **컨텍스트 기반의 직관적인 UX:** 플레이어는 단 하나의 키(`F`)만으로 상황에 맞는 상호작용을 자연스럽게 수행할 수 있습니다. 이는 복잡한 조작 없이도 깊이 있는 게임플레이를 가능하게 합니다.
*   **안전한 동시성 관리:** `UIOwner` 변수를 이용한 간단한 상태 잠금만으로, 여러 플레이어가 동시에 제작 UI를 열어 발생하는 데이터 충돌 문제를 원천적으로 방지했습니다.
*   **명확한 단일 책임 원칙 (SRP):** `ACraftingStation`은 상태 관리 및 제작 로직, `UInventoryComponent`는 아이템 보관, UI 위젯은 상태 표시에만 집중하여 각 객체의 역할이 명확하고 코드의 유지보수성이 높습니다.
*   **데이터 기반 확장성:** 기획자는 코드를 건드리지 않고 데이터 테이블 수정만으로 새로운 제작 레시피를 얼마든지 추가하거나 변경할 수 있습니다.
