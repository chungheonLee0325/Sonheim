...
> 이 문서는 [[4.4 팰 관리 시스템]]을 구성하는 컴포넌트들이 어떻게 유기적으로 협력하여, **서버 권위 하에 일관된 사용자 경험**을 보장하고, **확률적 연출**을 통해 게임적 재미를 더하는지 실제 코드를 통해 증명하는 기술 심층 분석 문서입니다.
...
이 모든 과정은 [[4.4 팰 관리 시스템|팰 관리 시스템]]을 구성하는 `UPalCaptureComponent`, `UPalInventoryComponent`, `UPalPartnerSkillComponent`와, [[6.1 통합 상호작용 원칙|상호작용 시스템]], [[3.3 스킬 시스템|스킬 시스템]], [[3.4 애니메이션 시스템|애니메이션 시스템]], 그리고 [[7.1 이벤트 기반 UI 아키텍처|UI 시스템]]의 유기적인 협력으로 이루어집니다.
...
## 8.2.5. 구현 경험: 경쟁 상태 방지를 위한 서버 상태 잠금
...
## 8.2.6. UI 연출 시퀀스의 지휘 (Orchestrating the UI Reveal Sequence)
